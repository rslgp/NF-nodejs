...
const default_nf = require("./NF_PlugNotas");

class NFSe_PlugNotas {
	async registrarNFSe(
		payloadDataMultiplasNotasJSONArray = mock.MOCK_payloadDataMultiplasNFSeJSONArray
	) {
		let api_endpoint_path = util.BASE_URL + "/nfse";
		return default_nf.registrarNF(
			api_endpoint_path,
			payloadDataMultiplasNotasJSONArray
		);
	}

	//retorna rps (recibo provisorio de servico)
	async consultarNFSe(id_NFSe) {
		//id_NFSe from current.id, sandbox id 5eef8cbc1666445f0598c958
	...
	async cancelarNFSe(id_NFSe, args) {
		...
		var payload = JSON.stringify({
			...
			message:
				"É necessário o body ao cancelar uma NFe, para incluir o código de cancelamento da cidade.",
		});
	...
	async consultarDadosNFSe_CNPJ(idIntegracao, cnpj) {
		let api_endpoint_path =
			util.BASE_URL + `/nfse/consultar/${idIntegracao}/${cnpj}`;
	...
	async cadastrarCertificado(args) {
		...
		const arquivoBlob = new Blob([args.arquivo.buffer], {
...
module.exports = new NFSe_PlugNotas();
